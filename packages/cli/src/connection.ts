...
function startHandshake(
  ...
  console.log(
    `📡 Attempting to reach ${targetId} (session: ${sessionId.slice(0, 8)}...)`,
  );
  ...
  const onSignal = ({
    ...
    if (signalData.type === "answer") {
      console.log(
        `   Answer SDP received, length: ${signalData.sdp?.length || 0}`,
      );
    }
    try {
  ...
  // In the peer creation, add ice connection state logging
  peer.on("iceConnectionStateChange", () => {
    console.log(`❄️ CLI ICE connection state: ${peer.iceConnectionState}`);

    if (peer.iceConnectionState === "connected") {
      console.log("✅ CLI ICE connected!");
    } else if (peer.iceConnectionState === "failed") {
      console.log("❌ CLI ICE failed - possible TURN/STUN issue");
    }
  });

  peer.on("iceGatheringStateChange", () => {
    console.log(`❄️ CLI ICE gathering state: ${peer.iceGatheringState}`);
  });
  ...
  peer.on("connect", () => {
    ...
    if (host) {
      ...
      import("./storage.js")
        .then(({ updateLastConnected }) => {
          updateLastConnected(targetId);
        })
        .catch((err) => {
          console.warn(
            "Could not update lastConnected in storage:",
            err.message,
          );
        });
    }
    ...
    setTransport(
      (packet: any) => {
        ...
        if (serialised.length > MAX_SERIALIZED_SIZE) {
          console.error(
            `❌ Cannot send: message too large (${serialised.length} bytes > ${MAX_SERIALIZED_SIZE})`,
          );
          return;
        }
    ...
    import("./transport.js").then(({ sendCommand }) => {
      sendCommand("identify", { clientId })
        .then((result: any) => {
          if (result?.ltmFacts > 0) {
            console.log(
              `🧠 Server loaded ${result.ltmFacts} LTM facts for this client`,
            );
          }
        })
        .catch(() => {
...
export async function startClient(
  ...
  socket.on(
    "registered",
    ({ role, sessionId }: { role: string; sessionId: string }) => {
